...
filter(
    fare_amount > 0 | tip_amount > 0) %>%
mutate(
    total = fare_amount + tip_amount)
...
ggmap(manhattan, darken = 0.5) +
    scale_fill_viridis(option = 'plasma') +
    stat_summary_2d(data = taxi, aes(x = long, y = lat, z = pred_total), bins = 60, fun = mean, alpha = 0.6) +
    labs(title = "Predicted Taxi fare", x = "long", y = "lat", fill = "Avg. trip price")
...
ggmap(manhattan, darken = 0.5) +
    scale_fill_viridis(option = 'plasma') +
    stat_summary_2d(data = taxi, aes(x = long, y = lat, z = total), bins = 60, fun = mean_if_enough_data, alpha = 0.6) +
    labs(title = "Actual Taxi fare", x = "long", y = "lat", fill = "Avg. trip price")
